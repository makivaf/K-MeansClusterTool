"""Construct the unimputed, participant-level ADNI study-entry table.
...
def validate(
    ...
    if table["RID"].duplicated().any():
        raise AssertionError("Study-entry table contains duplicate RID rows.")
...
def main() -> None:
    ...
    table.to_csv(INTERIM_DIR / "study_entry_cohort_unimputed.csv", index=False, encoding="utf-8")
    missing_audit.to_csv(INTERIM_DIR / "study_entry_missingness_audit.csv", index=False, encoding="utf-8")
    phase_audit.to_csv(INTERIM_DIR / "study_entry_phase_missingness.csv", index=False, encoding="utf-8")
    duplicates.to_csv(INTERIM_DIR / "study_entry_duplicate_qc.csv", index=False, encoding="utf-8")
    decisions.to_csv(INTERIM_DIR / "study_entry_exclusion_decisions.csv", index=False, encoding="utf-8")

    print(f"Eligible study-entry participants: {eligible_count}")
