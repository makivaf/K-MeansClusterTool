"""Construct the locked longitudinal observation cohort without slopes.

The script creates only the validated observation-level time axis authorized for
It does not fit regressions, calculate participant slopes, standardize,
reduce dimensions, select k, cluster, or alter raw/study-entry artifacts.
"""
...
OUTPUT_COHORT = ROOT / "data" / "interim" / "unified_longitudinal_cohort.csv"
OUTPUT_VALIDATION = ROOT / "data" / "interim" / "unified_longitudinal_cohort_validation.json"
...
def main() -> None:
    ...
    clustering_files: list[Path] = []
    for pattern in ("study_entry_*", "clustering_*", "enhanced_kmeans_*", "baseline_*", "dpc_*"):
        clustering_files.extend(sorted((ROOT / "data" / "interim").glob(pattern)))
    clustering_files.append(ROOT / "data" / "interim" / "unified_cluster_assignments.csv")
    clustering_files = sorted(set(clustering_files))
    clustering_manifest_before = hash_manifest(clustering_files)
    ...
    validation = {
        ...
        "provenance": {
            ...
            "clustering_artifacts_hash_verified": clustering_manifest_before,
            "locked_rules": {
        ...
        "participant_counts": {
            ...
            "final_longitudinal_participants": int(cohort["PTID"].nunique()),
        },
        ...
        "integrity": {
            ...
            "clustering_artifact_manifest_unchanged": True,
        },
        "outputs": {
    ...
    clustering_manifest_after = hash_manifest(clustering_files)
    if raw_hash_before != raw_hash_after:
        raise AssertionError("Raw ADAS changed during cohort construction")
    if clustering_manifest_before != clustering_manifest_after:
        raise AssertionError("A clustering artifact changed during cohort construction")
    ...
    print(json.dumps({
        ...
        "source_and_clustering_hashes_unchanged": True,
