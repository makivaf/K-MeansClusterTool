...
def safe_axis_b_example(field: str, series: pd.Series) -> tuple[str, str]:
...
def axis_b_ordering_assessment(field: str) -> tuple[str, str]:
...
def build_axis_b_rows(
    ...
    for field in AXIS_B_FIELDS:
        ...
        if found:
            ...
            example, value_format = safe_axis_b_example(field, series)
            ordering, ambiguity = axis_b_ordering_assessment(field)
            row.update(
                {
...
def main() -> None:
    ...
    axis_b_rows = build_axis_b_rows(raw_frames, inferred_frames)
    ...
    pd.DataFrame(axis_b_rows).to_csv(
        OUTPUT_DIRECTORY / "adni_axis_b_field_inventory.csv",
        index=False,
        encoding="utf-8",
    )
    ...
    print(f"Axis B inventory rows: {len(axis_b_rows)}")
