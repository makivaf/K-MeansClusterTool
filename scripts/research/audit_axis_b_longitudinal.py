"""Audit the longitudinal ADAS-Cog13 structure for Axis B without modeling.
...
AXIS_A_PATH = ROOT / "data" / "interim" / "axis_a_study_entry_unimputed.csv"
OUTPUT_PATH = ROOT / "data" / "interim" / "axis_b_adas_longitudinal_audit.json"
...
def main() -> None:
    for path in (ADAS_PATH, AXIS_A_PATH):
        if not path.is_file():
    ...
    if len(roster) != EXPECTED_ROSTER_SIZE or phase_counts != EXPECTED_ENTRY_PHASE_COUNTS:
        raise AssertionError(f"Axis A roster lock failed: rows={len(roster)}, phase_counts={phase_counts}")
    if roster["PTID"].duplicated().any() or roster["RID"].duplicated().any():
        raise AssertionError("Axis A in-scope roster does not have unique PTID and RID")
    ...
    report: dict[str, Any] = {
        "audit_scope": {
            ...
            "longitudinal_counts_population": "Axis A locked 2,437-person ADNI1/ADNIGO/ADNI2/ADNI3 roster, with only ADAS rows whose record PHASE is in those four phases.",
            "prohibited_outputs_created": [],
        },
        "files_inspected": {
            "ADAS": {"path": str(ADAS_PATH), "sha256": source_hash_before},
            "Axis_A_roster": {"path": str(AXIS_A_PATH), "sha256": file_sha256(AXIS_A_PATH)},
        },
        "basic_structure": {
            "rows": int(len(adas)),
        ...
        "phase_cohort_audit": {
            ...
            "Axis_A_all_ENTRY_PHASE_counts": {
                str(key): int(value)
                for key, value in sorted(roster_all["ENTRY_PHASE"].value_counts().to_dict().items())
            },
            "Axis_A_roster_rows": int(len(roster)),
            "Axis_A_ENTRY_PHASE_counts": {str(key): int(value) for key, value in sorted(phase_counts.items())},
            "Axis_A_unique_PTID": int(roster["PTID"].nunique()),
            "Axis_A_unique_RID": int(roster["RID"].nunique()),
        ...
        "data_quality_ordering_identifier_audit": {
            ...
            "traceability_assessment": "PTID and RID are suitable only if their observed one-to-one pairing checks pass; see counts above. Axis B joins use both fields together.",
        },
    }
