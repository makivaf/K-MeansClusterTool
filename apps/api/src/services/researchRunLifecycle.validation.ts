...
if (lifecycle.enqueue({ upload_ref: "upload-a" }, "upload-a-directory").run_id !== first.run_id) throw new Error("Duplicate admission created another job");
...
if (lifecycle.enqueue({ upload_ref: "upload-a" }, "upload-a-directory").run_id !== first.run_id || calls !== 2) throw new Error("Completed admission did not reconnect to its result");
...
if (formatResearchFailureDiagnostic(new ResearchExecutionError("EXECUTION_TIMEOUT", "PTID=SECRET")) !== "EXECUTION_TIMEOUT" || failedCleanup !== 0) throw new Error("Failure diagnostic or retry upload retention failed");
const retried = timeoutLifecycle.enqueue({ upload_ref: "upload-timeout" }, "timeout-directory");
if (retried.run_id === timedOut.run_id) throw new Error("A failed run could not be retried");
await timeoutLifecycle.whenIdle();
...
let routeJob: ReturnType<ResearchRunJobRepository["create"]> | null = null;
const routeApp = express();
routeApp.use(express.json());
routeApp.use(createResearchRunsRouter({
  getForUpload: () => routeJob,
  enqueue: () => { routeJob = routeRepository.create(); return routeJob; },
...
try {
  ...
  const reconnect = await fetch(`${baseUrl}/api/research/runs`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ upload_ref: routeUpload.uploadRef })
  });
  if (reconnect.status !== 202 || ResearchRunResponseSchema.parse(await reconnect.json()).run.run_id !== posted.run_id) {
    throw new Error("Admission retry did not reconnect after upload cleanup");
  }
